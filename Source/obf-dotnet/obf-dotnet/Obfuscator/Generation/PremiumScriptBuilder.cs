...
namespace Obfuscator.Obfuscation.Generation
{
    public class PremiumScriptBuilder
    {
        ...
        string GetLuaGeneration() { if (!ObfuscationSettings.EnhancedOutput) { return (""); }; return ("Vestra_LUA_GENERATION"); }
...
end; {(this.ObfuscationSettings.MaximumSecurityEnabled ? "Vestra_MAX_SECURITY_END()" : "")}" + "\n";
...
local Position, ByteString = ({Utility.Utility.IntegerToTable(1)}), (function(ByteString) Vestra_LUA_GENERATION local C, D, E = (""""), (""""), ({{}}); local I = 1; local function K() local L = ToNumber(SubString(ByteString, I, I), 36); I = I + 1; local M = ToNumber(SubString(ByteString, I, I + L - 1), 36); I = I + L; return (M); end; C = Dictionary[K()]; E[1] = C; while (I < #ByteString) do local N = K(); if G[N] then D = G[N]; else D = C .. SubString(C, 1, 1); end; G[F] = C .. SubString(D, 1, 1); E[#E + 1], C, F = D, D, F + 1; end; return (Concatenate(E)); end)([==[Vestra_BYTECODE]==]);
...
local Information = Vestra_VM_TABLE; 

Vestra_LUA_GENERATION

return LoadString(SubString(ByteString, Position))(Information or (function(...) Vestra_LUA_GENERATION end), ...);";
...
//            Console.WriteLine(Process.StartInfo.WorkingDirectory);
            ...
            Source = Source.Replace("Vestra_VM_TABLE", $"({{{VMTable}}})").Replace("Vestra_BYTECODE", Compressed);
            ...
            if (true)
            {
                ...
                {
                    string Substring = Source.Substring(SearchPosition);
                    Match Match = Regex.Match(Substring, @"Vestra_LUA_GENERATION");
                    if (Match.Success)
                    {
